...
import { Search, ImageOff, ExternalLink, Sparkles } from "lucide-react";
...
export default function GoogleCategory() {
  ...
  const [selectedGoogleCat, setSelectedGoogleCat] = useState("");
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Map and optimize Google product categories for your products</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        {/* Left Filtering Sidebar */}
        <div className="bg-card rounded-xl p-5 card-shadow border border-border space-y-5 h-fit">
          <h3 className="font-semibold text-foreground">Filtering</h3>
          ...
          <div>
            <label className="text-sm font-medium text-foreground mb-1.5 block">Search Product</label>
            <Input placeholder="Search..." value={search} onChange={(e) => setSearch(e.target.value)} className="bg-secondary border-0" />
          </div>
          ...
          <div className="bg-card rounded-xl p-5 card-shadow border border-border">
            <div className="flex flex-wrap items-end gap-4">
              <div className="flex-1 min-w-[200px]">
                <label className="text-sm font-medium text-foreground mb-1.5 block">Select Google Category:</label>
                <select
                  className="w-full rounded-lg border border-border bg-secondary px-3 py-2 text-sm text-foreground"
                  value={selectedGoogleCat}
                  onChange={(e) => setSelectedGoogleCat(e.target.value)}
                >
                  <option value="">Select Google Category</option>
                  {googleCategories.map((c) => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <Button className="bg-primary text-primary-foreground">Apply</Button>
                ...
                <div className="p-3 space-y-1.5">
                  ...
                  <div className="pt-1.5 border-t border-border">
                    {product.googleCategory ? (
                      <div>
                        <span className="text-[10px] font-semibold text-success uppercase">Optimized Category</span>
                        <p className="text-xs text-foreground truncate">{product.googleCategory}</p>
                      </div>
                    ) : (
                      <span className="text-[10px] font-semibold text-destructive uppercase">No optimized category</span>
                    )}
                  </div>
                </div>
              </div>
