...
function calcHealthScore() {
  ...
  // weighted penalty: high=3pts, medium=1pt
...
function IssueRow({
  ...
  issue: { issue: string; products: number; percentage: string };
...
function PrioritySection({
  ...
  priority: keyof typeof PRIORITY_CONFIG;
  issues: { issue: string; products: number; percentage: string }[];
}) {
  const [open, setOpen] = useState(priority === "high" || priority === "medium");
  const cfg = PRIORITY_CONFIG[priority];
  const Icon = cfg.icon;
  const resolved = 0; // mock — no resolved issues yet
...
export default function FeedAudit() {
  ...
  const chartData = (Object.keys(feedAuditIssues) as (keyof typeof feedAuditIssues)[]).map((k) => ({
    name: PRIORITY_CONFIG[k].label,
    value: feedAuditIssues[k].length,
    color: PRIORITY_CONFIG[k].chartColor,
  }));

  // quick wins = low % issues (easiest to fix first)
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          label="Total Issues"
          value={totalIssues}
          sub={`Across ${TOTAL_PRODUCTS} products`}
          icon={AlertCircle}
        ...
        <StatCard
          label="High Priority"
          value={feedAuditIssues.high.length}
        ...
        <StatCard
          label="Medium Priority"
          value={feedAuditIssues.medium.length}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-1 bg-card border border-border rounded-xl p-5 flex flex-col gap-4 card-shadow">
          ...
          <div className="space-y-2">
            {(Object.keys(feedAuditIssues) as (keyof typeof feedAuditIssues)[]).map((k) => (
        ...
        <div className="lg:col-span-2 bg-card border border-border rounded-xl p-5 card-shadow">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              ...
              <Tooltip
                formatter={(val: number, name: string) => [`${val} issues`, name]}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {quickWins.map((w, i) => (
              <div key={i} className="flex items-center gap-3 bg-success/5 border border-success/20 rounded-lg p-3">
                ...
                <Button size="sm" variant="outline" className="h-7 text-xs border-success/40 text-success hover:bg-success/10 shrink-0">
                  Fix
                </Button>
              </div>
            ))}
      ...
      <div className="space-y-3">
        ...
        {(Object.keys(feedAuditIssues) as (keyof typeof feedAuditIssues)[]).map((priority) => (
          <PrioritySection
            key={priority}
            priority={priority}
            issues={feedAuditIssues[priority]}
          />
        ))}
      </div>
