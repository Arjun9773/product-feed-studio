...
const AI_SUGGESTIONS = {
  ...
  "Age Group":  Object.fromEntries(mockProducts.map((p) => [p.id, { value: "Adult",  confidence: 98 }])),
  "Gender":     Object.fromEntries(mockProducts.map((p) => [p.id, { value: "Unisex", confidence: 97 }])),
  "Material": {
  ...
  "Pattern": Object.fromEntries(mockProducts.map((p) => [p.id, { value: "Solid", confidence: 95 }])),
  "GTIN": {
...
function ProductCard({ product, ps, onAccept, onReject, onManualChange, onManualSave }) {
  const borderClass =
          ...
          <AnimatePresence>
            <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
              ...
              <div className="flex gap-1.5">
                <Button size="sm" className="flex-1 h-7 text-xs bg-success/10 text-success border border-success/30 hover:bg-success/20" variant="ghost" onClick={onAccept}>
                  <Check className="h-3 w-3 mr-1" /> Accept
                </Button>
                <Button size="sm" className="flex-1 h-7 text-xs" variant="outline" onClick={onReject}>
        ...
        {ps.state === "rejected" && (
          <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="space-y-1.5">
            ...
            <div className="flex gap-1.5">
              ...
              />
              <Button size="sm" className="h-7 px-2 text-xs bg-primary text-primary-foreground shrink-0" onClick={onManualSave} disabled={!ps.manualValue.trim()}>
...
export default function FieldOptimization() {
      ...
      setProductStates((prev) => {
        ...
        mockProducts.forEach((p) => {
          if (next[p.id].state === "empty") {
            ...
            if (s) next[p.id] = { ...next[p.id], state: "suggested", suggestion: s.value, confidence: s.confidence };
          }
        });
        return next;
      });
  ...
  function handleAcceptAll() {
    setProductStates((prev) => {
      ...
      Object.keys(next).forEach((id) => {
        const ps = next[+id];
        if (ps.state === "suggested") next[+id] = { ...ps, state: "accepted", finalValue: ps.suggestion };
      });
      return next;
    });
  }
  ...
  function handleRejectAll() {
    setProductStates((prev) => {
      ...
      Object.keys(next).forEach((id) => {
        const ps = next[+id];
        if (ps.state === "suggested") next[+id] = { ...ps, state: "rejected" };
      });
      return next;
    });
  }
          ...
          <div className="bg-card rounded-xl p-4 card-shadow border border-border">
            <div className="flex flex-wrap items-end gap-3">
              <div className="flex-1 min-w-[180px] space-y-1.5">
                ...
                <select className="w-full rounded-lg border border-border bg-secondary px-3 py-2 text-sm text-foreground" value={selectedField} onChange={(e) => handleFieldChange(e.target.value)}>
                  {dataFields.map((f) => <option key={f}>{f}</option>)}
                </select>
              </div>

              <div className="flex items-end gap-2 flex-wrap">
                <Button className="gap-2 bg-purple-600 hover:bg-purple-700 text-white" onClick={handleAiFill} disabled={aiLoading}>
                  {aiLoading
                ...
                {suggested > 0 && (
                  <>
                    <Button variant="outline" className="gap-2 text-success border-success/40 hover:bg-success/10" onClick={handleAcceptAll}>
                      <CheckCheck className="h-4 w-4" /> Accept All
                    </Button>
                    <Button variant="outline" className="gap-2 text-destructive border-destructive/40 hover:bg-destructive/10" onClick={handleRejectAll}>
            ...
            {aiLoading && (
              <div className="mt-3 h-1 w-full bg-secondary rounded-full overflow-hidden">
                <motion.div className="h-full bg-purple-500 rounded-full" initial={{ width: "0%" }} animate={{ width: "100%" }} transition={{ duration: 1.5, ease: "easeInOut" }} />
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((product) => (
              <ProductCard
                ...
                onAccept={() => update(product.id, { state: "accepted", finalValue: productStates[product.id].suggestion })}
                onReject={() => update(product.id, { state: "rejected" })}
                onManualChange={(v) => update(product.id, { manualValue: v })}
                onManualSave={() => update(product.id, { state: "accepted", finalValue: productStates[product.id].manualValue })}
              />
            ))}
          </div>
        </div>
