...
function auditProduct(product) {
  ...
  if (
    ...
    issues.push({ field: 'brand', label: 'Brand not in title', priority: 'high', status: 'issue' });
  }
  ...
  if (!isEmpty(title)) {
    ...
    if (isAllCaps || isAllLower) {
      issues.push({ field: 'proper_casing', label: 'Proper casing', priority: 'medium', status: 'issue' });
    }
  }
